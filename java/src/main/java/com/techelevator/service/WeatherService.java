...
//        System.out.println(API_KEY);
    ...
    public WeatherDto getWeather(LatLon latLon, int userId){
        ...
        WeatherDto dto = new WeatherDto(0, userId, Integer.parseInt(latLon.getZip()), LocalDate.now(),
                response.getMain().getTemp(), response.getMain().getFeelsLike(), response.getMain().getHumidity(),
                response.getWeather()[0].getDescription(), response.getWeather()[0].getIcon(), latLon.getName());
        return dto;
    }
